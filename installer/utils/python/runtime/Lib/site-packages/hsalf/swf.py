...
LATIN, JAPANESE, KOREAN, SIMPLIFIED_CHINESE, TRADITIONAL_CHINESE = list(range(1, 6))
...
class FileHeader(SwfObject):
	...
	def _get_compressed(self):
		if self.signature[0] == b'C'[0]:
			return True
		return False
	def _set_compressed(self, b):
		if b:
			self.signature = b'CWS'
		else:
			self.signature = b'FWS'
...
class DoABCTag(Tag):
	def __init__(self):
		self.tag_code = DO_ABC
		self.flags = 0
		self.name = String()
		self.bytes = b''
	
	def _deserialize(self, f, version=0, *args, **kwargs):
		a = f.tell()
		self.flags, = struct.unpack('<I', f.read(4))
		self.name = String().deserialize(f, version)
		l = f.tell() - a
		self.bytes = f.read(self.tag_length - l)
	
	def _serialize(self, f, version=0, *args, **kwargs):
		f.write(struct.pack('<I', self.flags))
		self.name.serialize(f, version)
		f.write(self.bytes)

class SymbolClassEntry(SwfObject):
	def __init__(self):
		self.id = 0
		self.class_name = String()
	
	def deserialize(self, f, version=0, *args, **kwargs):
		self.id, = struct.unpack('<H', f.read(2))
		self.class_name = String().deserialize(f, version)
		return self
	
	def serialize(self, f, version=0, *args, **kwargs):
		f.write(struct.pack('<H', self.id))
		self.class_name.serialize(f, version)

class SymbolClassTag(Tag):
	def __init__(self):
		self.tag_code = SYMBOL_CLASS
		self.symbols = []
	def _deserialize(self, f, version=0, *args, **kwargs):
		n_symbols, = struct.unpack('<H', f.read(2))
		self.symbols = [SymbolClassEntry().deserialize(f, version) for i in range(n_symbols)]
	
	def _serialize(self, f, version=0, *args, **kwargs):
		f.write(struct.pack('<H', len(self.symbols)))
		for i in self.symbols: i.serialize(f, version)
...
class ClipActions(SwfObject):
	...
	def deserialize(self, f, version=0, *args, **kw_args):
		...
		while True:
			...
			if look_ahead == b'\x00' * look_ahead_len:
				break
...
class SwfFile(SwfObject):
	...
	decoders = {
		...
		SYMBOL_CLASS: SymbolClassTag,
		DO_ABC: DoABCTag,
	}

	def __init__(self, file_name=None, body=False):
	...
	def load_header(self, file_name):
		...
		self.file = file_name
		should_close = False
		if isinstance(file_name, str) or isinstance(file_name, str):
